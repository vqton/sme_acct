...
describe('JWT', () => {
  ...
  describe('token expiry', () => {
    it('access token expires in 15 minutes', () => {
      const token = generateToken({ userId: 'u1', username: 'test', roles: [] });
  ...
  describe('token claims', () => {
    ...
    it('includes jti (token id) claim', () => {
      const token = generateToken({ userId: 'u1', username: 'test', roles: [], jti: 'unique-id-123' });
      const payload = decodePayload(token);
    ...
    it('includes userId, username, and roles', () => {
      const token = generateToken({ userId: 'u1', username: 'test', roles: ['ke-toan-vien'] });
      const payload = decodePayload(token);
    ...
    it('generates unique jti when not provided', () => {
      const t1 = generateToken({ userId: 'u1', username: 'test', roles: [] });
      const t2 = generateToken({ userId: 'u1', username: 'test', roles: [] });
  ...
  describe('verifyToken', () => {
    it('returns decoded payload for valid token', () => {
      const token = generateToken({ userId: 'u1', username: 'test', roles: ['admin'] });
    ...
    it('rejects token signed with wrong secret', () => {
      const token = jwt.sign({ userId: 'u1' }, 'wrong-secret', { expiresIn: '15m' });
      expect(() => verifyToken(token)).toThrow();
    });
  });
});
