...
export function tenantMiddleware(req: TenantRequest, res: Response, next: NextFunction): void {
  ...
  ).all(req.user.userId) as { company_id: number }[]);

  req.userCompanyIds = rows.map((r) => r.company_id);

  const paramCompanyId = Number((req.params as Record<string, string>).companyId
...
export function filterByCompany<T extends { companyId?: number }>(
  items: T[],
